<template>
  <div style="display: inline;">
    <el-link v-if="type === 'create'" class="create-btn" icon="el-icon-edit" @click="dialogFormVisible = true">添加人员</el-link>
    <el-button v-if="type === 'edit'" type="text" @click="dialogFormVisible = true">{{ $t('common.edit') }}</el-button>

    <el-dialog :title="$t('research.project_admin_info.TITLE')" :visible.sync="dialogFormVisible" class="dialog">
      <el-form :model="person" label-width="100px" label-position="left">
        <el-form-item  :label="$t('research.project_admin_info.person')" class="form-item">
...
export default {
  ...
  methods: {
    ...
    postData(peopleId) {
      ...
      if (!this.loading) {
        this.loading = true
        createProjectAdmin(projectId, peopleId).then(res => {
          this.$message({
